...
@endsection
@section('content')
    <form action="{{ route('edit_year_post') }}" method="post" enctype="multipart/form-data" class="form-horizontal"
        autocomplete="off">
        @csrf
        <div class="card">
                        ...
                        <div class="row form-group row res_c">
                            ...
                            <div class="col-12 col-md-7">
                                <div class="row">
                                    ...
                                    <input type="text" class="col-4" id="a_and_d_year" value="{{ $data->atplan }}" name="atplan"
                                    value="{{ old('name')|$data->atplan }}" placeholder="กรอกปี" class="form-control" >
                        ...
                        @for ($i = 1; $i <= $data->atplan; $i++)
                            <div class="row form-group res_cc">
                                ...
                                <div class="col-12 col-md-9">
                                    <input {{($i!=1?'readonly':false)  }} compleate="{{  strlen((string) date("Y")+543)  }}" type="text" id="name-input" {{($i==1?'name=start':false) }}  value="{{$data->start+$i-1}}" placeholder="" class="form-control {{($i==1?'compleate-bf':'show_year_att')  }}">
                                   @if ($i==1)
...
<script src="https://code.jquery.com/jquery-3.6.0.js" integrity="sha256-H+K7U5CnXl1h5ywQfKtSj8PCmoN9aaq30gDh27Xc0jk=" crossorigin="anonymous"></script>
    <script>
        ...
        function createdivlis_year(count){
            ...
            $('.lenght_key').text($(".compleate-bf").val().length);
