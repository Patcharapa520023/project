...
class Strategicstb extends Controller
{
    public function show(){
        $tables = Strategics::with('user')->get()->toArray();
        $headtables  = array(
            array("ลำดับ","id"),
            // array("บทบาท","rolse"),
        ...
        return view('page2.strategics.table.table_strategics',compact('headtables'));
    }
    public function getdata(Request $request){
        ...
        $totalRecords = Strategics::with('user')->select('count(*) as allcount')->count();
        $totalRecordswithFilter = Strategics::with('user')->select('count(*) as allcount')->where('name', 'like', '%' .$searchValue . '%')->count();

          $records =Strategics::with('user')->orderBy($columnName,$columnSortOrder)
        ...
        foreach($records as $key=> $record){
                $formurl = route('delete_strategics_post');
                $id = $record->user->id;
